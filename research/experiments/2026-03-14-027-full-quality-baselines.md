# Experiment 027: Full Quality Baselines — JANG vs MLX on Qwen2.5-3B
...
| **JANG 8-bit** | **8** | **'The'** | **YES** | **15.38** | **matches** |
| MLX uniform 4-bit | 4.5 | 'The' | YES | 11.31 | MSE 11.31 |
| JANG 4-bit (RTN) | 4.0 | 'How' | NO | 12.04 | broken |
| MLX uniform 2-bit | 2.5 | 'izu' | NO | 8.56 | MSE 17.57 |

## The Challenge

JANG at 2.5-bit average must beat MLX uniform 4-bit (MSE < 11.31).

Current status:
- JANG 4-bit RTN: WORSE than MLX 4-bit (wrong token)
- JANG 8-bit: matches reference perfectly

## Why JANG 4-bit RTN Fails While MLX 4-bit Works

1. **MLX uses effective 4.5 bits** (group_size=64 adds scale+bias overhead).
   JANG uses exactly 4.0 bits average with some blocks at 2-3 bit.
...
The paper claim "JANG-2.5bit matches uniform 4-bit" requires:
...
This is the standard that EXL2 meets and what JANG must also meet.
