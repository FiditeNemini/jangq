...
MLX uses bf16 original weights. JANG uses our 2.76-bit quantized weights.

## Per-Layer Hidden State Norms (last position)

| Layer | JANG Norm | MLX Norm | Ratio | Diverging? |
|-------|----------|----------|-------|------------|
...
| JANG (2.76bit) | 251 ('') | 13.23 | [7.18, 5.40, 3.49, 3.99] |
...
1. Test JANG-4bit and JANG-6bit to see if higher bits fix the output
2. Test with activation-aware calibration
3. Try GPTQ-style quantization
4. Rule out forward pass bugs by testing with bf16 weights through JANG runtime
