"""Convert Zyphra/ZAYA1-VL-8B BF16 weights to a ZAYA1-VL MXFP4/JANG affine bundle."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert Zyphra/ZAYA1-VL-8B BF16 source to ZAYA1-VL MXFP4 affine bundle."
    )
...
def main() -> None:
    ...
    try:
        ...
        print("=" * 70)
        print(f"  Zyphra/ZAYA1-VL-8B -> MXFP{bits} affine conversion")
        print("=" * 70)
        print(f"  Source: {src}")
        print(f"  Output: {out}")
        ...
        for layer in tqdm(layers, desc="  Experts"):
            members = {
                expert: experts[(layer, expert)] for expert in range(n_experts) if (layer, expert) in experts
            }
            ...
            for proj, stack in stacks.items():
                base = f"model.layers.{layer}.zaya_block.experts.switch_mlp.{proj}"
        ...
        print("\n  Done!")
        print(f"  Affine groups:           {total_affine}")
