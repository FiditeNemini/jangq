...
class ZayaRouter(nn.Module):
    def __init__(self, args: ModelArgs, layer_idx: int):
        ...
        # MOD/skip-route bias initializer: when use_mod=True the last logit is
        # the skip-experts MOD route. Init it to -1.0 so the network only picks
        # the skip route when an expert is clearly worse; otherwise uniform-zero
        # init causes the bundle to over-route through MOD on the first forward.
