...
- [x] **M200 (Settings lie removed — `defaultCalibrationSamples` + `calibrationJSONL`)** — Iter-137 rotated to **angle I (Setting-actually-affects-behavior)** after 10 iters of angle-C security work. Picked `defaultCalibrationSamples` (E13 open item: "Calibration sample count setting — verify it actually passes to the Python convert CLI"). Traced end-to-end; confirmed it was a **100% lie** per `feedback_dont_lie_to_user.md`.
      ...
      **Commit:** (this iteration)
- [ ] **M201** — Broader Settings-lie audit (spawned from M200). Other candidates flagged during iter-137 trace:
      - `outputNamingTemplate` — `renderOutputName()` method exists but no non-Settings consumer found. If the convert pipeline always uses hardcoded `{basename}-{profile}`, this Settings field is a lie.
      - `autoDeletePartialOnCancel` — verify the cancel path actually reads this toggle.
      ...
      Every one of these is an angle-I candidate. Budget one iter per 2-3 fields. For each field: trace consumer, prove-or-disprove effect, REMOVE-or-PLUMB following the M200 protocol, pin invariant.
