...
- [x] **M90 (Cat D fourth pass)** — `feedback_readme_standards.md` rule 10: "YAML frontmatter must include `reasoning` AND `thinking` tags for all Qwen3.5 models". `detect_capabilities` in `examples.py:87` exposed only `has_reasoning = bool(reasoning_parser) or bool(enable_thinking)` — ORed both signals into ONE flag. The model-card template then emitted only a single `reasoning` YAML tag. The separate `thinking` tag that memory requires was NEVER emitted, even on Qwen3.5 models where `enable_thinking: true` is explicitly set in config.json. Every Qwen3.5 HF upload violated the standard.
      ...
      **Commit:** (this iteration)
- [ ] **M91** — `feedback_readme_standards.md` documents 10+ HARD requirements for HF uploads that the auto-generated template does NOT cover: per-subject MMLU scores (need live evals), JANG-vs-MLX side-by-side, speed comparison, size comparison, Korean section, per-subject comparison table. The template produces a skeleton that would FAIL the memory's rules if published directly. Should either (a) add explicit `<!-- TODO: run MMLU -->` placeholder sections that make the gap visible, or (b) add a GenerateModelCardSheet warning banner "auto-generated card is a starting point; add evals before publishing." Not a bug — a documentation/UX gap.
