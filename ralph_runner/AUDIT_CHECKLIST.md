...
- [x] **M-audit (iter 44)** — Cat D memory cross-ref pass across 4 project memories (`project_qwen36.md`, `project_mistral4_architecture.md`, `project_minimax_m27.md`, `project_glm51_jang1l_working.md`). Verified each claim against current code, no drift found. (Last Cat D was iter 38 — 6-iter cadence, due now.)
      ...
      **Commit:** (this iteration)
- [ ] **M121** — In-wizard smoke-test (`TestInferenceSheet` via `InferenceRunner` → Python `jang_tools inference`) does not pass `enable_thinking=False` to `apply_chat_template`. For reasoning models (GLM-5.1, Qwen3.6, MiniMax M2.7) this means a 150-token smoke test budget is consumed by `<think>...` and the user never sees an answer — looks like the model is broken. Per `project_glm51_jang1l_working.md`: "enable_thinking=False is REQUIRED for short factual prompts. Without it, GLM-5.1 enters <think>...</think> reasoning that eats 100+ tokens before emitting the final answer." Fix scope (flagged iter 44, deferred to keep iter scope clean): (a) add `--no-thinking` flag to `inference.py` CLI, (b) add `noThinking: Bool = false` param to `InferenceRunner.generate`, (c) add toggle to `TestInferenceSheet` / `TestInferenceViewModel`, (d) tests on all three layers. Default behavior unchanged; opt-in for smoke-test UX. (Do NOT flip the default — would surprise users running reasoning benchmarks.)
