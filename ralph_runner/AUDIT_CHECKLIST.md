...
- [ ] **M70** — State.json read-during-write race. `Path.write_text` is NOT atomic (open → write → close); a concurrent `--status` could see truncated JSON. Status doesn't take the lock (intentional — reads should be free). Switch `save_state` to temp-file + `os.rename` (atomic on APFS).
- [ ] **M71** — `cmd_reset` doesn't take the lock. If `--next` is converting and user runs `--reset`, state.json gets nuked mid-convert. Reset should acquire the lock (and fail with BLOCKED if held), OR at minimum warn before deleting a state with running combos.
