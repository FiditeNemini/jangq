...
- [ ] **M81** — `ralph_runner/fixtures/` hosts the VL/video fixtures for a11/a12. Need to verify these are git-tracked (not .gitignored) — otherwise a fresh clone can't run those audits and the check silently degrades to n/a on every first run.
- [ ] **M82** — `run_audits` has no per-row timeout. a15 can hang indefinitely on a deadlocked `mlx_lm.load` (e.g., corrupted shard, missing file, Metal kernel stall). Should wrap each row in a timeout context, configurable per row (a15 = 5 min default, a17/a18 = 60s default since they shell out to jang-tools).
