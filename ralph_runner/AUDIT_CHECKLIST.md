...
- [ ] **M39** — `AUDIT_CHECKLIST` category K (Ralph runner) — still empty. Ralph runner on macstudio hasn't been traced end-to-end. Does it actually cancel a 2-hour convert cleanly when the user cancels? Does it stream progress back via `progress.py` or swallow it?
...
- [ ] **M44** — `PublishResult.swift` doesn't decode the `commit_url` field that `publish.py` emits on success. The commit URL is the MOST USEFUL info (links directly to the upload in HF). Users have to navigate manually to their profile to verify. Add `commitUrl: String?` to PublishResult.
...
- [ ] **M46** — No repo-name validation on Swift side. User enters "my model/repo" (space) or "foo" (no `org/name`) → `huggingface_hub.create_repo` raises a cryptic InvalidRepoIdError. Add a regex check + actionable hint ("Repo id must be 'org/name' format, lowercase, no spaces") before dispatching.
