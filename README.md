> **Note on research:** Detailed research documentation — including MoE expert sensitivity analysis, MLP asymmetry findings, FP8 dequantization methods, and per-layer precision rules — is no longer published here. Within days of our findings being shared publicly, a competing project introduced nearly identical techniques (expert down_proj protection, VLM vision passthrough, FP8 source handling) without acknowledgment. We continue to open-source the quantization engine and format. Our [commit history](https://github.com/jjang-ai/jangq/commits/main) timestamps every discovery for those interested in provenance.
