> **Note on research:** Detailed research documentation — including MoE expert sensitivity analysis, MLP asymmetry findings, FP8 dequantization methods, and per-layer precision rules — is no longer published here. These findings came from hundreds of hours of empirical testing across 6+ model architectures, dozens of failed conversions, and careful layer-by-layer analysis that can't be arrived at through theory alone. Within days of our results being shared publicly, a competing project introduced nearly identical techniques (expert down_proj protection at the exact same bit offset, VLM vision passthrough, FP8 source handling for the same models) without acknowledgment. We continue to open-source the quantization engine and format — we believe in building openly, not taking someone else's hard-won results and rebranding them as your own — and we hope others in this community hold themselves to the same standard. Our [commit history](https://github.com/jjang-ai/jangq/commits/main) timestamps every discovery for those interested in provenance.
